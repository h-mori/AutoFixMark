...
def determine_selected_indices(hits: List[Row], top_n: int, relaxed_threshold_ratio: float = None):
    """
    Determine which hit indices should be selected for KO output.
    Returns set of indices that should be selected.

    If relaxed_threshold_ratio is specified, hits without asterisk are only selected
    if score/thrshld >= relaxed_threshold_ratio.
    """
    selected_indices = set()
    has_asterisk = any(hit.asterisk for hit in hits)

    for index, hit in enumerate(hits):
        # Ratio check: if relaxed_threshold_ratio is specified, check score/thrshld ratio
        if relaxed_threshold_ratio is not None and hit.thrshld > 0:
            passes_ratio_check = (hit.score / hit.thrshld) >= relaxed_threshold_ratio
        else:
...
def format_detail_output(gene_data: Dict[str, List[Row]], top_n: int, detail_top: int, relaxed_threshold_ratio: float = None):
    """
    ...
    # Write header
    output_lines.append("selected\trank\tsignificant\tgene name\tKO\tthrshld\tscore\tE-value\t\"KO definition\"")
    ...
    for i, (gene, hits) in enumerate(gene_data.items()):
        ...
        selected_indices = determine_selected_indices(hits, top_n, relaxed_threshold_ratio)
        ...
        for index, hit in enumerate(hits):
            if index < detail_top or hit.asterisk:
                ...
                selected = 'Y' if index in selected_indices else ''
                significant = '*' if hit.asterisk else ''
                
                # Output: selected, rank, significant(asterisk), then all original columns except the first one
                cols_str = '\t'.join(hit.original_columns[1:])
                output_lines.append(f"{selected}\t{original_rank}\t{significant}\t{cols_str}")

    return output_lines


def format_ko_output(gene_data: Dict[str, List[Row]], top_n: int, relaxed_threshold_ratio: float = None):
...
def format_gene_output(gene_data: Dict[str, List[Row]], top_n: int, relaxed_threshold_ratio: float = None):
    """Generate output lines for gene mode with detailed hit information."""
    output_lines = []

    # Write header
    output_lines.append("KO\tgene name\tthrshld\tscore\tE-value\tsignificant")
...
def parse_kofamscan_result_file(input_file: str, output_file: str, top_n: int = 1, relaxed_threshold_ratio: float = None):
    ...
    ko_lines = format_ko_output(gene_data, top_n, relaxed_threshold_ratio)
    with open(output_file, 'w', encoding='utf-8') as out:
        if ko_lines:
            out.write('\n'.join(ko_lines))

    # Output 2: Gene details
    gene_lines = format_gene_output(gene_data, top_n, relaxed_threshold_ratio)
    with open(gene_output_file, 'w', encoding='utf-8') as out:
        if gene_lines:
            out.write('\n'.join(gene_lines))

    # Output 3: Detail view (detail_top is fixed at 10)
    detail_lines = format_detail_output(gene_data, top_n, detail_top=10, relaxed_threshold_ratio=relaxed_threshold_ratio)
...
def main():
    ...
    parser.add_argument('--relaxed-threshold-ratio',
                        type=float,
                        default=None,
                        help='Relaxed score/threshold ratio (0 < ratio < 1) for selecting hits without asterisk. If not specified, no ratio filtering is applied.')
    ...
    # Validate relaxed_threshold_ratio range
    if args.relaxed_threshold_ratio is not None:
        if not (0 < args.relaxed_threshold_ratio < 1):
            print("Error: --relaxed-threshold-ratio must be greater than 0 and less than 1", file=sys.stderr)
            sys.exit(1)
    ...
    parse_kofamscan_result_file(args.input_file, args.output_file, args.top, args.relaxed_threshold_ratio)
